...
class ConfigParams(models.Model):
    ...
    CP_HotKeyWord = models.CharField(max_length=20,
                                     default='NT', blank=False, verbose_name='热搜关键字')
    CP_TopicsLimit = models.IntegerField(
        default=100, blank=False, verbose_name='书评获取数量')
    CP_CommentsLimit = models.IntegerField(
        default=100, blank=False, verbose_name='书评评论获取数量')
    ...
    CP_TopicsPageLimit = models.IntegerField(
        default=10, blank=False, verbose_name='每页书评数量')
...
class User(AbstractUser):
    ...
    UT_TopicsCount = models.IntegerField(verbose_name='书评发布数量', default=0)
    ...
    UT_TreplayCount = models.IntegerField(verbose_name='书评评论数量', default=0)
...
class TopicArticleStatistic(models.Model):
    ...
    TAS_ID = models.UUIDField(
        primary_key=True, auto_created=True, default=uuid.uuid4, verbose_name='书评ID')
    TAS_Title = models.CharField(
        max_length=35, unique=True, verbose_name='评分')
    TAS_Description = models.TextField(max_length=140, verbose_name='书评描述')
    ...
    TAS_Theme = models.CharField(
        max_length=100, default="其他", verbose_name='书评主题')
    TAS_Type = models.ForeignKey(
        CategoryInfo, to_field='CI_Name', on_delete=models.CASCADE, verbose_name='书评类别')
    ...
    class Meta:
        verbose_name = '书评信息'
        # 末尾不加s
        verbose_name_plural = '**1**书评基础信息**1**'
...
class ArticleTags(models.Model):
    AT_TAID = models.UUIDField(
        default=uuid.uuid4, null=False, editable=False, verbose_name='书评ID')
...
class ArticleComment(models.Model):
    ...
    AC_ArticleID = models.ForeignKey(
        TopicArticleStatistic, to_field='TAS_Title', on_delete=models.CASCADE, verbose_name='书评ID')
    # AC_ArticleID = models.UUIDField(
    #    null=False, editable=True, verbose_name='书评ID')
...
class ArticleUserLikesOrDislikesTable(models.Model):
    ...
    class Meta:
        verbose_name = '立场记录'
        # 末尾不加s
        verbose_name_plural = '**1**书评立场统计**1**'
...
class ArticleReadsIP(models.Model):
    ...
    class Meta:
        verbose_name = '阅读IP记录'
        # 末尾不加s
        verbose_name_plural = '**1**书评阅读IP统计**1**'
...
class UserCollect(models.Model):
    ...
    UC_Article = models.ForeignKey(
        TopicArticleStatistic, to_field='TAS_ID', on_delete=models.CASCADE, verbose_name='书评ID')
    UC_CollectTime = models.DateField(auto_now=True, verbose_name='时间')

    class Meta:
        verbose_name = '书评收藏'
        verbose_name_plural = '**1**书评用户收藏**1**'
