...
def PaginatorInfoGet(objects, number, URLParams):
    if objects:
        ...
        return {'ObjectList': ObjectList, 'ObjectsPaginator': ObjectsPaginator,
                'Paginator_num_pages': Paginator_num_pages, 'Paginator_Href': Paginator_href}
    else:
        return {'ObjectList': [], 'ObjectsPaginator': '', 'Paginator_num_pages': 0, 'Paginator_Href': ''}


def NoticeCount(request):
    return str(QRC('Notice.objects.filter(TargetUser=%s).count()', None,
                   request.user)) if request.user.is_authenticated else '0'
...
def GetTopic(request):
    if request.user.is_authenticated:
        ...
        DataDict['Rating'] = TopicObject.Rating
        themes = []
...
def AddTopic(request):
    if request.user.is_authenticated:
        InsertDataDict = {'Category': request.POST.get('Category'),
        ...
        try:
            ...
            InsertDataDict['Content'] = mMs.MovePicToSavePath(InsertDataDict['Content'])
            # 如果Topic有值则为编辑书评
            if TopicID:
                mMs.RemovePicFromSavePath(TopicID, InsertDataDict['Content'])
                Topic = TopicInfo.objects.get(ObjectID=TopicID)
                Category = QRC('TopicCategoryInfo.objects.get_or_create(Name=%s)', None,
                               InsertDataDict['Category'])[0]
                RatingAll = Category.RatingNumber * Category.Rating - Topic.Rating + InsertDataDict['Rating']
                Category.Rating = RatingAll / Category.RatingNumber
                Category.save()
                Topic.Content = InsertDataDict['Content']
                Topic.Description = InsertDataDict['Description']
                Topic.Rating = InsertDataDict['Rating']
                Topic.Theme.clear()
                Topic.Theme.add(*ThemeObjects)
                Topic.save()
                return HttpResponse('ok')
            else:
                # 不用QRC的原因是ContentText书评中的引号容易出现问题!
                Topic = TopicInfo.objects.create(ObjectID=mMs.CreateUUIDstr(),
                                                 Publisher=request.user,
                                                 Category=QRC('TopicCategoryInfo.objects.get_or_create(Name=%s)', None,
                                                              InsertDataDict['Category'])[0],
                                                 Title=request.user.Nick + "对" + InsertDataDict[
                                                     'Category'] + "的书评",
                                                 Content=InsertDataDict[
                                                     'Content'],
                                                 Description=InsertDataDict[
                                                     'Description'],
                                                 Rating=InsertDataDict[
                                                     'Rating'],
                                                 )
                RatingAll = Topic.Category.RatingNumber * Topic.Category.Rating + InsertDataDict['Rating']
                Topic.Category.RatingNumber += 1
                Topic.Category.Rating = RatingAll / Topic.Category.RatingNumber
                Topic.save()
                Topic.Category.save()
                Topic.Theme.clear()
                Topic.Theme.add(*ThemeObjects)
            ...
            if BlackListRecord:
                ...
                try:
                    NewRollCall = QRC('RollCallInfo.objects.create(Title=%s,Publisher=%s,Target=%s,ObjectID=%s)', 0,
                                      RollCallTitle, request.user,
                                      QRC('User.objects.get(Nick=%s)', None, TargetUserNick), mMs.CreateUUIDstr())
                    NewDialogue = QRC(
                        'RollCallDialogue.objects.create(RollCallID=%s,Publisher=%s,Content=%s,ObjectID=%s)', 0,
                        NewRollCall, request.user, RollCallContent, mMs.CreateUUIDstr())
...
def ContextConfirm(request, **Params):
    ...
    # 标签信息获取
    ThemeList = QRC('TopicThemeInfo.objects.all()', None)
    # 推荐发布者信息获取
    PublisherList = QRC("PublisherList.objects.all()", None)
    # 生成上下文字典
    ContextDict = {"Layout_Sizer": Params['URLParams'],
                   "Main_URL_Sizer": {'Topic': ('is-active', '', ''), 'RollCall': ('', 'is-active', ''),
                                      'SpecialTopic': ('', '', 'is-active'), 'UserProfile': ('', '', ''),
                                      'Search': ('', '', ''),
                                      'Category': ('', '', '')}[Params['URLParams']['Region']],
                   ...
                   # "ExportList_Themes": ThemeList,
                   "ExportList_Publishers": PublisherList,
                   "Paginator_num_pages": Params['PaginatorDict'][
                       'Paginator_num_pages'] if 'PaginatorDict' in Params else '',
                   ...
                   "ExportItem_CategoryInfo": Params['Category'] if 'Category' in Params else '',
                   "Is_Published": '0'
                   }
    if (ContextDict["ExportItem_CategoryInfo"]):
        for Card in ContextDict["ExportList_Cards"]:
            if Card[0].Publisher_id == request.user.id:
                ContextDict["Is_Published"] = Card[0].ObjectID
    return ContextDict
    ...
    if request.user.is_authenticated:
        ...
        if record and len(record) < 2:
            ...
            QRC(('Topic' if Type in 'SpecialTopic' else 'Comment') +
                'Attitude.objects.create(ObjectID=%s,Type=%s,Publisher=%s,Point=%s)', 0, Object, Type, request.user,
                int(Point))
...
def Replay(request):
    ...
    if request.user.is_authenticated:
        if Type in 'SpecialTopic':
            ReplayObject = QRC(
                'CommentInfo.objects.create(ObjectID=%s, TopicID=%s,Content=%s,Parent=%s,Type=%s,Publisher=%s)',
                0, mMs.CreateUUIDstr(), TopicObject, Content,
                QRC('CommentInfo.objects.get(ObjectID=%s)', None, ParentID), Type, request.user)
        else:
            RollCall = QRC(
                'RollCallInfo.objects.get(ObjectID=%s)', None, ObjectID)
            ReplayObject = QRC(
                'RollCallDialogue.objects.create(ObjectID=%s,RollCallID=%s,Content=%s,Display=%s,Publisher=%s)',
                0, mMs.CreateUUIDstr(), RollCall, Content, '' if RollCall.Publisher == request.user else 'right',
                request.user)
...
def UserActive(request, username, key):
    ...
    return render(request, APPConf.ActivePage,
                  {'AlertInfo': message, 'RedirectURL': '127.0.0.1:8000' if settings.DEBUG else 'www.nagetive.com'})
...
def Login(request):
    ...
    username = mMs.Decrypt(mMs.DecodeWithBase64(temp))
...
def Regist(request):
    ...
    try:
        ...
        newUser.Avatar = mMs.UserAvatarOperation(request.POST.get(
            ...
            APPConf.DefaultAvatar.url.replace(settings.MEDIA_URL, ''))['Path']
        newUser.save()
        return HttpResponse('ok')
...
def UserProfileUpdate(request):
    ...
    if QRC('User.objects.get(Nick=%s)', 0, UserNickName) and QRC('User.objects.get(Nick=%s)', 0,
                                                                 UserNickName) != request.user:
        return HttpResponse('Nick')
    else:
        UploadImage_Operated = ''
        UploadImage_Operated = mMs.UserAvatarOperation(UserImageData, UserImageFormat, userObject.Avatar)
